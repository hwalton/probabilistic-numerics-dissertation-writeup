...
\begin{document}
    ...
    The decomposition of the input and response functions from the time domain to the frequency domain uses a linear operator called the Fourier Transform, $\mathcal{F}(\xi)$, as defined in Equation~\ref{eq:ft}
    ...
    \nomenclature[G07]{$\boldsymbol{\mu}_X$  $\boldsymbol{\mu}_*$}{Vectors containing the mean of the distribution of values of the latent function, at the training points, $\mathbf{X}$, and test points $\mathbf{X}_*$ respectively}
    ...
    \nomenclature[G08]{$\boldsymbol{\mu}_{*\vert \mathbf{X}}$}{Conditional mean of the Gaussian Process, given the training data, $\mathbf{X}$}
    \nomenclature[G15]{$\boldsymbol{\Sigma}_{*\vert \mathbf{X}}$}{Conditional covariance of the Gaussian Process, given the training data, $\mathbf{X}$}
    ...
    \nomenclature[G09]{$\boldsymbol{M}_*$}{The mean of the frequency domain prediction function}
...
%    \end{equation}
    ...
    This allows one to convert to the frequency domain information about the uncertainty of vibrations, as shown in Figure~\ref{fig:stdv-plot}.
    Here, the standard deviation is highest around $0 \, rad \, s^{-1}$, and tapers off for higher frequencies.
    ...
    Therefore, one could not confidently conclude that the acceleration will be below the fatigue limit without a conservative factor of safety.
    In contrast, ``rather than relying on large factors of safety to account for uncertainty''~\cite{Rogers2023} which could add unnecessary weight to overly strengthen the component, the uncertainty information is encoded in the GP model's standard deviation of the posterior distribution.
    Therefore, it would be possible to calculate the probability that the acceleration falls over the limit.
    If this chance is sufficiently low, one can conclude that the component is below the fatigue limit without the need for a conservative safety factor, increasing the performance of the engine.


    \subsection{Computation Time}
    However, one major disadvantage of this GP model is that the computation time scales poorly with large datasets, especially the hyperparameter training phase since it is of $O(N^3)$\nomenclature[L24]{$O()$}{Big O notation, used to describe the complexity - or ``Order'' of an algorithm}, which is poor compared to $O(N \log{N})$ for the Fast Fourier Transform (FFT).
