...
\begin{document}
    ...
    However, it can only be applied in its base form to a function that is continuous in the domain $-\inf$ to $\inf$, meaning that it has a value defined at every point in time.
    ...
    A numerical method % finish: more generally...
    ...
    But most importantly, the FFT can be highly sensitive to noise in the data, meaning a small amount of noise cancan create large fluctuations in the shape of the frequency spectrum, masking the true underlying frequencies and modal properties\cite{MathStackExchange2023}.
    ...
    Moreover, it is defined in the continuous domain $-\inf$ to $\inf$ which means that the noise resilience of an FRF computed using this method may be improved because this can be transformed by the continuous Fourier Transform instead of resorting to the discrete FFT which is highly sensitive to noise.
    ...
    Incredibly, since the Fourier Transform is also a linear transformation, the result of this transformation of a GP is yet another GP\@.
    ...
    An alternative method of modelling error that is more robust to noise would have great implications more generally in engineering because it could be implemented in other contexts where numerical methods would classically be chosen to perform transformations of functions.
