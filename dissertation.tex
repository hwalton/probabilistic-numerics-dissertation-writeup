...
\begin{document}
    ...
    After this, the function for the Fourier Transform of the mean prediction was constructed using the method described by Ambrogioni et al~\cite{Ambrogioni2018}, which could be evaluated at any set of points in the domain $(-\infty, \infty)$.
    ...
    Successful implementation can be shown if the NLML is the same for the basic GP and its sparse approximation when the set of inducing points is exactly the same as the test dataset.
    ...
    + Stdv function of the fourier transform and its applications can be explored
