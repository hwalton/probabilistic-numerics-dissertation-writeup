...
%    \end{longtable}
    ...
    \nomenclature[L]{$y$}{A vector of $y_n$ values from a set of training points}
    \nomenclature[L]{$f^*$}{A vector of values of the latent function evaluated at a set of test points, $X_*$}
    \nomenclature[L]{$I$}{Identity matrix}
    \nomenclature[L]{$K_{X,X}$}{Covariance matrix for training input data, $K(X,X)$}
    \nomenclature[L]{$K_{X,*}$}{Covariance matrix between training inputs and test inputs, $K(X,X_*)$}
    \nomenclature[L]{$K_{*,*}$}{Covariance matrix for test inputs, $K(X_*,X_*)$}
    \nomenclature[G]{$\epsilon_n$}{Noise in observation, normally distributed with a mean of zero and a variance of $\sigma^2_y$}
    \nomenclature[G]{$\mu_X$}{Mean of the Gaussian Process, given the training data $X$}
    \nomenclature[G]{$\mu_*$}{Mean of the Gaussian Process, given the test data $X_*$}
    ...
    Note that block matrix notation is used where matrices are conjoined to form a larger block matrix. \nomenclature[L]{$K_{A,B}$}{Covariance matrix of the kernel function, $k(a,b)$, evaluated for each pair of elements a, b in vectors A, B}\nomenclature[L]{$k$}{Kernel function of arbitrary points, $a$, and $b$}
    ...
    By a process called conditioning the joint Gaussian prior distribution on the observations~\cite{rasmussen2006gaussian}, the ``posterior'', or prediction of the latent function given our set of sampled training points~\cite{murphy2023probabilistic} is shown in Equation~\ref{eq:18.51}, where $\mu_{*|X}$ and $\Sigma_{*|X}$ are the conditional mean and covariance of the GP, given the training data, given in Equation~\ref{eq:18.52} and Equation~\ref{eq:18.53} respectively.
    ...
    \nomenclature[G]{$\mu_{* \vert X}$}{Conditional mean of the Gaussian Process, given the training data}
    \nomenclature[G]{$\Sigma_{* \vert X}$}{Conditional covariance of the Gaussian Process, given the training data}
    ...
    \end{equation}
    \nomenclature[L]{$K_{ij}$}{Element of a covariance matrix, K, at index i, j}
