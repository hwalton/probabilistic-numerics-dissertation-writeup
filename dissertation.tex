...
%    Note that $k(- \xi, 0)$ is the original time domain kernel since the Fourier Transform applied to a function twice is equivalent to the original function reflected about zero and scaled by dividing by $2 \pi$.
    ...
    The accuracy of the Fourier Transform computed by the GP was first compared to that of the FFT algorithm using a basic dataset of a free vibration SDOF system with damping, but with no noise added to validate the new approach of computing the Fourier Transform.
    ...
    Since the Fourier Transform is a continuous function, it can be sampled at any set of points in the domain $(-\infty, \infty)$.
     So, taking the basic dataset and clustering an additional $100$ samples of the spectrum of the GP in the interval of $1 \, rad \, s^{-1}$ containing the peak was able to improve the error in the calculation of the angular frequency of the peak from the same as the FFT ($0.0149 \, rad \, s^{-1}$) down to $0.0062 \, rad \, s^{-1} $.
    ...
    This creates an uncertainty of $\pm$ half the sample interval.
