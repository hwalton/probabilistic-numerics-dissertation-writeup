...
%        $\sigma^2$ & Variance, a hyperparameter in the context of the periodic kernel \\
...
%    \end{longtable}
    ...
    \nomenclature[L]{$f^*$}{An array of values of the latent function evaluated at a set of test points, $X_*$}
    \nomenclature[L]{$I$}{Identity matrix}

    where $f^*$ is an array of values of the latent function evaluated at a set of test points $X_*$, $I$ is the identity matrix and $K_{X,X}$, $K_{X,*}$ \& $K_{*,*}$ are a type of covariance matrix, $K(A,B)$, based on the ``kernel'' function, $k(a,b)$, when evaluated for each pair elements in the arrays $(X,X)$, $(X,X_*)$ and $(X_*,X_*)$ respectively. \nomenclature[L]{$K_{A,B}$}{Covariance matrix of the kernel function, $k(a,b)$, evaluated for each pair of elements a, b in arrays A, B}\nomenclature[L]{$k$}{Kernel function of arbitrary points, $a$, and $b$} \nomenclature[L]{$K_{X,X}$}{Covariance matrix for training input data, $K(X,X)$} \nomenclature[L]{$K_{X,*}$}{Covariance matrix between training inputs and test inputs, $K(X,X_*)$} \nomenclature[L]{$K_{*,*}$}{Covariance matrix for test inputs, $K(X_*,X_*)$}
