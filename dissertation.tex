...
\begin{document}
    ...
    \begin{enumerate}[listparindent=\parindent]
        ...
        Studying this dynamic system would be beneficial as it may allow for the prediction of behaviour based on varying these parameters.
    ...
    Similarly, many algorithms make use of mathematical objects (such as matrix pseudoinverses) which have multiple alternatives where the correct choice is subjective.
    ...
    \noindent where $\omega$ is the angular frequency and $f(t)$ is the time domain function being transformed.
    This is an extremely powerful analytical technique that is used widely in engineering.
    However, it for it to be applied in its base form to a function, this function must be defined for every point in time in the continuous domain $-\infty \text{ to } \infty$.
    ...
    \noindent where $\epsilon_n$ is the observational noise, $\mathbf{y}$ is a vector of $y_n$ values from a set of training points, $\boldsymbol{\mu}_X$ and $\boldsymbol{\mu}_*$ are a vectors containing the mean of the distribution of values of the latent function, at the training points, $\mathbf{X}$, and test points $\mathbf(X)_*$ respectively. $\mathbf{f}_*$ is a vector of function values at a set of test points $\mathbf{X}_*$, $\mathbf{I}$ is the identity matrix and $\mathbf{K}_{X,X}$, $\mathbf{K}_{X,*}$ \& $\mathbf{K}_{*,*}$ are a type of covariance matrix, $\mathbf{K}(\mathbf{A},\mathbf{B})$, based on the ``kernel'' function, $k(a,b)$ that evaluated for each pair elements in the vectors $(\mathbf{X},\mathbf{X})$, $(\mathbf{X},\mathbf{X}_*)$ and $(\mathbf{X}_*,\mathbf{X}_*)$ respectively.
    ...
    Furthermore, the noise term $\epsilon_n$ explicitly models the observational noise, giving the GP a way to distinguish between the signal and noise components.
    ...
    \end{equation}
    \nomenclature[L]{$k(a,b)$}{kernel function evaluating similarity of points $a$, $b$}
    \nomenclature[L]{$a, b$}{Input points of kernel function k(a,b)}
    \nomenclature[L]{$\sigma^2$}{Variance, a hyperparameter in the context of the kernel}
    \nomenclature[L]{$l$}{Length scale parameter of the SE kernel}

    \noindent where $k(a,b)$ is the kernel, evaluated at elements $a$, $b$ of vectors $\mathbf{A}$, $\mathbf{B}$ respectively; $\sigma^2$ is the variance hyperparameter and $l$ is the length scale parameter.
    ...
    \begin{equation}
        \text{MSE} = \frac{1}{n} \sum_{i=1}^{n} (a_i - a_i)^2
        \label{eq:mse}
    \end{equation}

    \nomenclature[L]{$A_i$, $B_i$}{Values of vectors $\mathbf{A}$, $\mathbf{B}$ at index i}

    \noindent where $a_i$, $a_i$ are the values of vectors $\mathbf{A}$, $\mathbf{B}$ at index i.
