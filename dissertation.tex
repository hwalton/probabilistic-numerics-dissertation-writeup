...
\begin{document}
    ...
    We are using a Gaussian Process (GP) to map a continuous ``latent`` function $f(x)$ to our noisy discrete sampled data that takes a time as an input and outputs in a force.
    ...
    \subsection{The Application of the Gaussian Process in Modal Analysis}

    This investigation is in the context of modal analysis of a Multi-Degree of Freedom (MDOF) system, which is when the vibration of a structure is analysed by finding what is known as the Frequency Response Function (FRF). This is defined as the frequency domain response divided by the frequency domain input force to the structure, which is used when finding properties of the system known as damping ratios, resonant frequencies, and mode shapes (the shapes that the structure vibrates at each resonant frequency).
    ...
    The overarching aim can be broken down into four objectives:
        \begin{enumerate}
            ...
            \item Stretch Objective: Compare the smoothing out of sharp resonance peaks due to spectral leakage.
        \end{enumerate}
