...
\begin{document}
    ...
    \nomenclature[L]{$c$}{Damping Coefficient}
    \nomenclature[L]{$k$}{stiffness}
    ...
    \end{figure}
    \nomenclature[G]{$\xi_{r}$}{Resonant Frequency}
    \nomenclature[G]{$\zeta$}{Damping Ratio}
    ...
    \end{equation}
    \nomenclature[L05]{$\mathcal{F}[f](\xi)$}{Fourier Transform of $f(t)$}
    ...
    \nomenclature[L09]{$\mathbf{I}$}{The identity matrix}
    ...
    \nomenclature[L10]{$\mathbf{K}$}{A covariance matrix of the kernel function, $k(a,b)$, evaluated for each pair of elements a, b in vectors $\mathbf{A}$, $\mathbf{B}$}
    \nomenclature[L]{$_{X,X}$  $_{X,*}$  $_{*,*}$}{Subscripts that denote that the matrix is formed from the function evaluated for each pair elements in the vectors $(\mathbf{X},\mathbf{X})$, $(\mathbf{X},\mathbf{X}_*)$ and $(\mathbf{X}_*,\mathbf{X}_*)$ respectively}
    ...
    \end{equation}
    \nomenclature[L]{$t_k$}{The $k^{th}$ element in the vector of training points $\mathbf{X}$}
    \nomenclature[G]{$\mu(t)$}{The mean function of the Gaussian Process in the time domain}
    \nomenclature[L]{$\boldsymbol{\mathcal{F}}[\mu]_*$}{A vector of the Fourier Transform of the mean function, evaluated for vector of test points $X_*$}
    \nomenclature[L]{$\mathbf{\mathcal{F}}_*$  $\mathbf{\mathcal{F}}_{*1}$  $\mathbf{\mathcal{F}}_{*2}$}{Matrices of Fourier Transforms (with respect to $t$, $t_1$ and $t_2$ respectively}
    \nomenclature[L]{$t_1$ $t_2$}{Independent elements of vector $\mathbf{X_{*}}$}
...
%    \end{figure}
    ...
    \nomenclature[G06]{$\sigma^2$}{The variance hyperparameter of the squared exponential kernel}
    \nomenclature[L16]{$l$}{Length scale hyperparameter of the squared exponential kernel}
...
%%    \end{figure}
    ...
    \end{equation}
    \nomenclature[L165]{$\mathcal{K}(\xi,0)$}{Fourier Transform of the Squared Exponential Kernel function evaluated at $\xi$}
    ...
    \end{equation}
    \nomenclature[L]{$\mu(t)$}{The mean function}
    ...
    \end{equation}
    \nomenclature[L]{$\Sigma(t, t)$}{The covariance function of the Gaussian Process in the time domain}
    \noindent where $v_k$ is defined in Equation~\ref{eq:v_k}

    \begin{equation}
        v_k = \sum_j a_{kj} \, k(t,t_j) \label{eq:v_k}
    \end{equation}
    \nomenclature[L]{$t_j$}{The $j^{th}$ element in the vector of training points $\mathbf{X}$}
...
%    and is always positive and real (see Mercer's theorem~\cite{rasmussen2006gaussian}, p.96), meaning that its phase is always zero.
