...
%    In this paper, the Gaussian process is used for 1D regression, meaning it maps one continuous input to one continuous output.
%    \subsubsection{Bayes Theorem}
%    A discrete event is an event that has a finite set of possible values (such as the value of a throw of dice).
%    The Gaussian process is built upon a special form of Bayes Theorem that describes the covariance of a set of continuous random variables, meaning how the probability distribution function of one depends of discrete events when the probabilities depend on each other~\cite{stuart1994kendalls}, as shown in Equation~\ref{eq:bayes}.
%
%    \begin{equation}
%        P(y|X) = \frac{P(X|y) \cdot P(y)}{P(X)}\label{eq:bayes}
%    \end{equation}
%
%    where:
%    \begin{itemize}
%        \item $P(y|X)$ is the Posterior, which we are trying to find.
%        It is the probability of y being
    ...
    \begin{equation}
        -\log p(D|\theta) = \frac{1}{2} (y - \mu_X)^T K_{\sigma}^{-1} (y - \mu_X) + \frac{1}{2} \log |K_{\sigma}| + \frac{N}{2} \log(2\pi)
    \end{equation}
