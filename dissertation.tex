...
\begin{document}
    ...
    A comparison was made by calculating the Mean Squared Error (MSE) with respect to the analytically exact Fourier Transform for each.
    ...
    A more accurate Fourier Transform allows for more accurate Frequency Response Functions, which would have widespread benefits in Mechanical Engineering.
    ...
    The \textcolor{red}{GP was shown to have better robustness to noise, with an error in wn of xxx versus xxx for the FFT}

    A Fourier Transform that is more robust to noise would be more accurate and therefore would be beneficial in the context of Mechanical Engineering as explained in Section~\ref{Acc}, but would also improve other areas of engineering where it is used extensively such as control theory, signal processing, image processing, and acoustic engineering.
    ...
    \subsection{Non-Uniform Sampling of the Time Domain and Overcoming the Nyquist Limit}
    ...
    \section{Conclusions}
    Overall, when compared to the industry standard Fast Fourier Transform (FFT), the alternative Gaussian Process (GP) model proposed showed \textcolor{red}{better??} robustness to noise as well as several promising features that make it more versatile.
    ...
    This gives the model the potential to exceed the performance of the FFT in industrial contexts where there are constraints to the amount of data that can be collected due to cost or availability.
    However, the computational speed of this model is currently orders of magnitude slower than the FFT, so before the GP model can be a viable alternative to the FFT in practice, it would need to be developed to scale better to large datasets.
