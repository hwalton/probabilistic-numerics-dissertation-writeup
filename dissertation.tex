...
\begin{document}
    ...
    The noise term $\epsilon_n$ explicitly models the observational noise, giving the GP a way to distinguish between the signal and noise components.
    ...
    The kernel is a function that produces a covariance matrix when input with two vectors.
    This measures the similarity between each pair of inputs based on a set of hyperparameters and is chosen to reflect any prior knowledge about the problem domain.
    However, the function must be carefully chosen to ensure that the resulting matrix is symmetrical about the main diagonal and has a property called Positive Semi-definiteness.
