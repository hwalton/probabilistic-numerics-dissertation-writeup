...
\begin{document}
    ...
    Next, the function for the Fourier Transform of the prediction of the mean was constructed using the neat method described by Ambrogioni et al~\cite{Ambrogioni2018}, which could be evaluated at any set of points in the domain $(-\infty, \infty)$.
    This is an equivalent method to conditioning the joint distribution in Equation~\ref{eq:linear-trans} which instead takes the Fourier Transform of the time domain predictive mean function directly.
    It did this by taking the Fourier Transform of the kernel function only.
    Here, the Fourier shift theorem was first applied to ensure that the kernel was a function of a single variable.
    This was possible because the Squared Exponential Kernel used is stationary, and allowed a remarkably simple expression for the Fourier Transform to be found analytically, as shown in Equation~\ref{eq:se-kernel-ft}

    \begin{equation}
        k(\omega,0) = \frac{\sigma^2 l}{\sqrt{2 \pi}} \exp\left(-\frac{l^2 \omega^2}{2}\right) \label{eq:se-kernel-ft}
    \end{equation}
    \nomenclature[L165]{$k(\omega,0)$}{Fourier Transform of the Squared Exponential Kernel function evaluated at $\omega$}

    \noindent where $\omega$ is the frequency and $k(\omega,0)$ is the Fourier Transform of the Squared Exponential Kernel function evaluated at $\omega$.
