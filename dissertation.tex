...
\begin{document}
    ...
    \begin{equation}
        F(\xi) = \frac{1}{\sqrt{2 \pi}} \int_{-\infty}^{\infty} f(t) e^{-i \xi t} \, dt\label{eq:ft}
    \end{equation}
    ...
    This overarching aim can be broken down into three objectives:
        \begin{enumerate}
            \item Create a working Gaussian Process model, which involves:
                ...
                \end{itemize}
            \item Adjust this model for the frequency domain by finding the Fourier Transform of the mean and covariance functions in closed form.
    ...
    \begin{equation}
        \mathcal{F}[\Sigma](\xi) = \frac{1}{2 \pi} e^{i \xi^2} k(- \xi,0)  - \sum_k \sum_j a_{kj} \, e^{-i \xi (t_j - t_k)} \mathcal{K}(\xi,0)^2 \label{eq:var-sum-freq-domain}
    \end{equation}

    Note that $k(- \xi, 0)$ is the original time domain kernel since the Fourier Transform applied to a function twice is equivalent to the original function reflected about zero and divided by $2 \pi$.

    This method was only able to produce the main diagonal of the covariance matrix, not the whole matrix, but this was sufficient since hyperparameter training was completed in the time domain already, and only the main diagonal was required to plot the standard deviation of the output predictions (which is the square root of this main diagonal).
    In fact, only calculating the main diagonal was preferred since it decreased the compute requirements significantly.
    ...
    \subsubsection{Non-uniform Sampling of Frequency Domain}
    ...
    \subsubsection{Non-Uniform Sampling of the Time Domain and Overcoming the Nyquist Limit}
    ...
    \textcolor{red}{results data mse for non-uniform and 1 datapoint missing??}
    ...
    Finally, the fact that time intervals need not be uniform and that the spectrum is avaialable in closed form allows frequencies to be detected that would be above the nyquist frequency for the DFT with the same number of datapoints and the same total sampling period... \textcolor{red}{data}
    ...
    Unlike the DFT, the standard deviation of the prediction of the spectrum can be found using the GP, as shown in \textcolor{red}{Figure ... is plotted in Figure ...}
    This is derived from the main diagonal of the covariance matrix.
    Note that it only appears in the magnitude plot, not the phase plot since it is positive and real (see Mercer's theorem~\cite{rasmussen2006gaussian}, p.96), meaning that its phase is always 0.
    Since each amplitude prediction is a gaussian distribution, some portion of it will always be negative.
    However, since amplitudes are usually positive only, this can be interpreted as a positive amplitude of the same magnitude but the opposite phase.
    The uncertainty has a peak around zero and gradually reduces for higher frequencies, which makes intuitive sense because lower frequency vibrations will experience fewer cycles in a given time period, so there will be less information available to determine its magnitude.
    ...
    In the automotive industry, a piston connecting (``con'') rod in an internal combustion engine undergoes heavy vibration, so fatigue analysis is crucial to ensure that it does not fail from crack formation and propagation before the end of the design life of the vehicle.
    ...
    However, if the FFT was used to convert to the frequency domain, the uncertainty information would not directly be provided and so it would not be clear whether this uncertainty would cause the upper bound of the range of acceleration possible given the data to fall above the limit.
    ...
    Therefore, it would be possible to calculate the probability that the acceleration falls over the limit and if this chance is sufficiently low, one can conclude with confidence that the component is below the fatigue limit.
    ...
    This means that the model handles poorly datasets consisting of more than a few thousand points, and even at this scale, fitting the GP to the time domain then converting this to the frequency domain takes around 4 orders of magnitude longer than the FFT, which is nearly instantaneous.
    This performance is poor considering that it is not uncommon to need to work with datasets consisting of up to a million datapoints, for which this GP model in its current state would be unsuitable.

    To address this, the Sparse GP approximation known as the Fully Independent Training Conditional (FITC) could be employed to allow both the NLML and predict functions to scale to large data~\cite{q-candela}.
    ...
    It was attempted extensively but the implementations did not produce NLML values consistent with the basic GP and thus failed to converge to fit the test data.
    ...
    \textcolor{red}{and another subsection??:: New kernel types; multi degree of freedom / dimension GP's?; different solver types? etc.}
