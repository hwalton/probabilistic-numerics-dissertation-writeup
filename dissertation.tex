...
\begin{document}
    ...
    The Gaussian Process (GP) a type of machine learning algorithm that uses noisy, discrete sampled data to model a continuous ``latent'' function, $f(x)$ that represents the underlying relationship between an input, $x$, and an output, $f(x)$.\nomenclature[L]{$f(x)$}{Latent function of the variable $x$}
