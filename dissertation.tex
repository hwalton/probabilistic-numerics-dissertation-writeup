...
\begin{document}
    ...
    The Gaussian Process (GP) a type of machine learning algorithm that fits a continuous ``latent'' function, $f(x)$ to noisy, discrete sampled data using probabilistic numerics.\nomenclature[L]{$f(x)$}{Latent function of the variable $x$}
    This represents the underlying relationship between an input, $x$, and an output, $f(x)$.
    In essence, a Gaussian Process is ``a collection of random variables, any finite number of which have a joint Gaussian distribution''~\cite{rasmussen2006gaussian} which describes how the probability distribution of each variable depends on the other variables.
    ...
    Since the prediction of the output is given as probability distribution, this provides an inherent mechanism for quantifying its uncertainty.
